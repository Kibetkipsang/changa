...
export function MyChamas() {
    ...
    const loadChamas = async () => {
      ...
      try {
        ...
        if (response.data.chamas) {
          console.log(
            `📊 Found ${response.data.chamas.length} chamas in response`,
          );
  ...
  const getRoleBadge = (role: string) => {
    ...
    return (
      <span
        className={`px-2 py-0.5 rounded-full text-xs font-medium ${styles[role] || styles.MEMBER}`}
      >
        {role}
      </span>
    );
        ...
        {userChamas.length > 0 ? (
                ...
                >
                  <div className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <ChevronRight
                        className={`w-5 h-5 text-gray-400 transition-transform ${hoveredChama === chama.id ? "translate-x-1 text-purple-600" : ""}`}
                      />
                    </div>
                    ...
                    <div className="space-y-3 mt-4">
                      ...
                      {chama.contributionAmount && (
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="font-medium text-gray-900">
                            KSh {chama.contributionAmount.toLocaleString()}
                          </span>
                        </div>
                      )}
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-6">
              Create a new chama or join an existing one to get started
            </p>
