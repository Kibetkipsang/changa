...
export function Loans() {
  ...
  const approveMutation = useMutation({
    mutationFn: async (loanId: string) => {
      const response = await api.patch(`/loans/approve/${currentChama?.id}/${loanId}`);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['loans', currentChama?.id] });
  ...
  const rejectMutation = useMutation({
    mutationFn: async ({ loanId, reason }: { loanId: string; reason: string }) => {
      const response = await api.patch(`/loans/reject/${currentChama?.id}/${loanId}`, { rejectionReason: reason });
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['loans', currentChama?.id] });
  ...
  const activateMutation = useMutation({
    mutationFn: async (loanId: string) => {
      const response = await api.patch(`/loans/activate/${currentChama?.id}/${loanId}`);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['loans', currentChama?.id] });
  ...
  const repaymentMutation = useMutation({
    mutationFn: async ({ loanId, amount, notes }: { loanId: string; amount: number; notes: string }) => {
      const response = await api.post(`/loans/repayment/${currentChama?.id}/${loanId}`, { amount, notes });
      return response.data;
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['loans', currentChama?.id] });
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'APPROVED':
        return <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800"><UserCheck className="w-3 h-3" /> Approved</span>;
      case 'ACTIVE':
      ...
      case 'DEFAULTED':
        return <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800"><XCircle className="w-3 h-3" /> Defaulted</span>;
      default:
    ...
    return (
      <Layout>
        <div className="text-center py-16 bg-white rounded-xl shadow-sm p-8">
          <p className="text-gray-600">Please select a chama to manage loans.</p>
        </div>
      </Layout>
    );
  }
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">Request, approve, and track loans for {currentChama.name}</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
            ...
            <p className="text-2xl font-bold text-gray-900">KSh {summary.totalLoanAmount?.toLocaleString() || 0}</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
            <p className="text-sm text-gray-500">Outstanding Balance</p>
            <p className="text-2xl font-bold text-yellow-600">KSh {summary.totalOutstandingBalance?.toLocaleString() || 0}</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="text-left px-6 py-3 text-sm font-semibold text-gray-600">Interest</th>
                  ...
                  <th className="text-left px-6 py-3 text-sm font-semibold text-gray-600">Requested</th>
              ...
              <tbody>
                ...
                ) : loans.filter((l: Loan) => l.user?.name?.toLowerCase().includes(searchTerm.toLowerCase())).length === 0 ? (
                  <tr><td colSpan={8} className="text-center py-12 text-gray-500">No loans found</td></tr>
                ) : (
                  loans.filter((l: Loan) => l.user?.name?.toLowerCase().includes(searchTerm.toLowerCase())).map((loan: Loan) => (
                    <tr key={loan.id} className="border-b hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-gray-600">{loan.interestRate}%</td>
                      <td className="px-6 py-4 text-gray-600">{loan.repaymentPeriod} months</td>
                      ...
                      <td className="px-6 py-4 text-gray-500 text-sm">{format(new Date(loan.createdAt), 'dd/MM/yyyy')}</td>
                            ...
                            <>
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  if (reason) rejectMutation.mutate({ loanId: loan.id, reason });
                                }}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-md w-full p-6">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Loan for {showRepaymentForm.user?.name} · Balance: KSh {showRepaymentForm.balance.toLocaleString()}
            </p>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium">KSh {selectedLoan.amount.toLocaleString()}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium text-yellow-600">KSh {selectedLoan.balance.toLocaleString()}</p>
                  </div>
                  <div>
                  ...
                  <div className="space-y-2">
                    {selectedLoan.repayments.map((repayment, idx) => (
                      <div key={idx} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                        <div>
                          <p className="font-medium">KSh {repayment.amount.toLocaleString()}</p>
