  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await api.get(
        `/contributions/${currentChama?.id}?${params}`,
      );
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: any }) => {
      const response = await api.put(
        `/contributions/${currentChama?.id}/${id}`,
        data,
      );
      return response.data;
    },
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
            ...
            <p className="text-2xl font-bold text-gray-900">
              KSh {summary.totalCollected?.toLocaleString() || 0}
            </p>
          </div>
          <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
            <p className="text-sm text-gray-500">This Month</p>
            <p className="text-2xl font-bold text-gray-900">
              KSh {summary.currentMonthCollected?.toLocaleString() || 0}
            </p>
          </div>
          <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
          ...
          <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
            ...
            <p className="text-2xl font-bold text-gray-900">
              KSh {summary.averageContribution?.toLocaleString() || 0}
            </p>
          </div>
        </div>
                    ...
                    .map((contribution: Contribution) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-gray-600">
                          {format(new Date(contribution.month), "MMMM yyyy")}
                        </td>
                        <td className="px-6 py-4">
