...
const API_BASE_URL = import.meta.env.VITE_API_URL || 'http://localhost:5000/changa';

export const api = axios.create({
  baseURL: API_BASE_URL,
  withCredentials: true, // CRITICAL: This sends/receives httpOnly cookies
  headers: {
...
export const contributionAPI = {
  create: (chamaId: string, data: any) => api.post(`/contributions/${chamaId}`, data),
  getAll: (chamaId: string, params?: any) => api.get(`/contributions/${chamaId}`, { params }),
...
export const loanAPI = {
  ...
  getAll: (chamaId: string, params?: any) => api.get(`/loans/${chamaId}`, { params }),
  getOne: (chamaId: string, loanId: string) => api.get(`/loans/${chamaId}/${loanId}`),
  ...
  reject: (chamaId: string, loanId: string, reason: string) => 
    api.patch(`/loans/reject/${chamaId}/${loanId}`, { rejectionReason: reason }),
...
export const meetingAPI = {
  create: (chamaId: string, data: any) => api.post(`/meetings/${chamaId}`, data),
  getAll: (chamaId: string, params?: any) => api.get(`/meetings/${chamaId}`, { params }),
