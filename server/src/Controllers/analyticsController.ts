...
export const getAnalytics = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const activeMembers = members.filter(
      (m) => m.role !== "MEMBER" || m.joinedAt > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
    ).length;
    ...
    const totalContributions = paidContributions.reduce(
      (sum, c) => sum + c.amount,
      0
    );
    ...
    const thisMonthContributions = paidContributions.filter(
      (c) => c.month >= startOfMonth
    );
    const monthlyContributionTotal = thisMonthContributions.reduce(
      (sum, c) => sum + c.amount,
      0
    );
    ...
    const membersWhoPaid = new Set(
      thisMonthContributions.map((c) => c.userId)
    );
