...
const getChamaId = (params: { chamaId?: string | string[] }) =>
  Array.isArray(params.chamaId) ? params.chamaId[0] : params.chamaId || "";
...
export const getEffectiveSettings = async (req: AuthRequest, res: Response) => {
  try {
    const chamaId = getChamaId(req.params);
    if (!chamaId) {
      return res.status(400).json({ error: "Invalid chamaId" });
    ...
    const effectiveSettings = {
      ...
      requiresBothApprovals: (amount: number) => {
        if (!settings.loanApprovalThreshold) return false;
...
export const validateLoanRequest = async (req: AuthRequest, res: Response) => {
  try {
    const chamaId = getChamaId(req.params);
...
export const getContributionRules = async (req: AuthRequest, res: Response) => {
  try {
